...
int main(int argc, char** argv) {
  ...
  while (true) {
    ...
    std::cout << "Now doing force alignment using output: " << wordPredictionStr << std::endl;
    // Now doing force alignment and printing the softmax values
    // calculate softmax for rawEmission
    auto paths = criterion->viterbiPathWithTarget(
    ...
    for(j=0; j<nframes; j++) {
      ...
      std::cout << "frame " << j << ": " << tokenDict.getEntry(mindex) << ":" << probs[mindex][j] << std::endl;
    }
